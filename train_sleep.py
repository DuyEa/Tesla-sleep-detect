...
# windows by folder labels (binary: close_look=1, others=0
def build_windows_by_folder_label(df: pd.DataFrame, window: int, ear_thresh: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for lbl in sorted(df["label"].unique()):
        ...
        for i in range(n_full):
            chunk = ears[i*window:(i+1)*window]
            miss = np.isnan(chunk)
            if np.all(miss):
                ears_imp = np.full_like(chunk, 0.25, dtype=float)
            else:
                med = np.nanmedian(chunk)
                ears_imp = np.where(miss, med, chunk)
            closed_flags = (ears_imp < ear_thresh).astype(int)
            ear_mean = float(np.mean(ears_imp))
            ear_std  = float(np.std(ears_imp))
            ear_min  = float(np.min(ears_imp))
            ecr      = float(np.mean(closed_flags))
            ...
            X = [ear_mean, ear_std, ear_min, ecr, miss_ratio, max_run]
            feats.append(X)
            targets.append(1 if lbl == "close_look" else 0)
    return np.array(feats, dtype=float), np.array(targets, dtype=int)
...
def main():
    ...
    # Build windows -> features + labels  (USING FOLDER LABELS instead of ECR/consec)
