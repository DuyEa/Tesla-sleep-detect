...
def head_pose_angles(lm2d, w, h):
    if lm2d is None: return None, None
    f = w
...
def main():
    ...
    # thresholds / rules
    ap.add_argument("--prob_thresh", type=float, default=0.5, help="RF probability threshold (distracted)")
    ap.add_argument("--distract_run_windows", type=int, default=3, help="Consecutive distracted windows => DISTRACTED")
    ap.add_argument("--high_ecr", type=float, default=0.80, help="Or DISTRACTED if ECR(window) ≥ this")
    ap.add_argument("--pitch_down_deg", type=float, default=25.0, help="Pitch (deg) to force DISTRACTED (phone)")
    ap.add_argument("--yaw_thresh", type=float, default=30.0, help="Abs yaw (deg) to force DISTRACTED (look away)")
    ap.add_argument("--miss_frames", type=int, default=20, help="No face/eyes for N frames => DISTRACTED")
    args = ap.parse_args()

    # New: interactive source choice if not provided by CLI
    ...
    pitch_win = deque(maxlen=window)
    yaw_win = deque(maxlen=window)
    distract_run = 0
    miss_ctr = 0
    last_prob = 0.0
    last_ecr, last_run = 0.0, 0
    last_pitch_mean, last_yaw_abs_mean = 0.0, 0.0
    ...
    with facemesh_ctx() as fm:
        while True:
            ...
            pitch_frame, yaw_frame = head_pose_angles(lm2d, W, H)
            if lm2d is None:
                miss_ctr += 1
                pitch_frame = yaw_frame = np.nan
            else:
                miss_ctr = 0

            ears_win.append(ear)
            pitch_win.append(pitch_frame)
            yaw_win.append(yaw_frame)
            ...
            if len(ears_win) == window:
                ...
                # Add pose features
                pitches_list = list(pitch_win)
                yaws_list = list(yaw_win)
                miss_p = np.isnan(pitches_list)
                if np.all(miss_p):
                    pitch_imp = np.zeros(window, dtype=float)
                else:
                    med_p = np.nanmedian(pitches_list)
                    pitch_imp = np.where(miss_p, med_p, pitches_list)
                pitch_mean = float(np.mean(pitch_imp))
                yaws_abs = np.abs(yaws_list)
                miss_y = np.isnan(yaws_abs)
                if np.all(miss_y):
                    yaw_abs_imp = np.zeros(window, dtype=float)
                else:
                    med_y = np.nanmedian(yaws_abs)
                    yaw_abs_imp = np.where(miss_y, med_y, yaws_abs)
                yaw_abs_mean = float(np.mean(yaw_abs_imp))
                last_pitch_mean, last_yaw_abs_mean = pitch_mean, yaw_abs_mean
                X = np.append(X, [pitch_mean, yaw_abs_mean])

                # RF probability (distracted)
                try:
                    prob = clf.predict_proba([X])[0][1]
                except Exception:
                    prob = float(clf.predict([X])[0])  # 0/1 if proba not available
                last_prob = prob
                distracted = (prob >= args.prob_thresh)

                # Consecutive windows OR high ECR => DISTRACTED (model rule)
                distract_run = distract_run + 1 if distracted else 0
                distracted_model = (distract_run >= args.distract_run_windows) or (ecr >= args.high_ecr)

                # Head pose fallback (phone or look away)
                distracted_pose = (pitch_mean >= args.pitch_down_deg) or (yaw_abs_mean >= args.yaw_thresh)

                # No-face/eyes fallback
                distracted_miss = (miss_ctr >= args.miss_frames)

                distracted = distracted_model or distracted_pose or distracted_miss
            ...
            cv2.putText(frame, f"Status: {state}    Distract Prob: {last_prob:.2f}",
                        (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)

            # Info line
            ear_disp = 0.0 if np.isnan(ear) else ear
            pitch_str = "NA" if np.isnan(last_pitch_mean) else f"{last_pitch_mean:.1f}°"
            yaw_str = "NA" if np.isnan(last_yaw_abs_mean) else f"{last_yaw_abs_mean:.1f}°"
            info = f"EAR: {ear_disp:.3f}   ECR(win): {last_ecr:.2f}   Run(win): {last_run}/{window}   Pitch: {pitch_str}   |Yaw|: {yaw_str}   Miss: {miss_ctr}"
            cv2.putText(frame, info, (10, top_bar_h + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (230,230,230), 2)

            # Big alert banner
            if state == "DISTRACTED":
                draw_banner(frame, "DISTRACTED! ALERT!", (0,0,255), bg=(0,0,0), y=top_bar_h + 70)

            cv2.imshow("Driver Focus Detection (EAR+ECR + Head Pose)", frame)
            if writer.isOpened():
                writer.write(frame)
